...
#define STATUS_ERROR_MASK (MSS_UART_OVERUN_ERROR | MSS_UART_PARITY_ERROR | \
        ...
        mss_uart_rzi_pulsewidth_t pw)
    {
        ...
        ((rxpol == MSS_UART_ACTIVE_LOW) ? (this_uart->hw_reg->MM1 &= ~EIRX_MASK) : (this_uart->hw_reg->MM1 |= EIRX_MASK));

        ((txpol == MSS_UART_ACTIVE_LOW) ? (this_uart->hw_reg->MM1 &= ~EITX_MASK) : (this_uart->hw_reg->MM1 |= EITX_MASK));

        ((pw == MSS_UART_3_BY_16) ? (this_uart->hw_reg->MM1 &= ~EITP_MASK) : (this_uart->hw_reg->MM1 |= EITP_MASK));
        /* Disable SmartCard Mode */
        this_uart->hw_reg->MM2 &= ~EERR_MASK;
    }
        ...
        mss_uart_endian_t endian)
    {
        ...
        if (MSS_UART_INVALID_ENDIAN > endian)
        {
            ...
            ((MSS_UART_LITTLEEND == endian) ? (this_uart->hw_reg->MM1 &= ~E_MSB_RX_MASK) : (this_uart->hw_reg->MM1 |= E_MSB_RX_MASK));
        }
    }
        ...
        mss_uart_endian_t endian)
    {
        ...
        if (MSS_UART_INVALID_ENDIAN > endian)
        {
            ...
            ((MSS_UART_LITTLEEND == endian) ? (this_uart->hw_reg->MM1 &= ~E_MSB_TX_MASK) : (this_uart->hw_reg->MM1 |= E_MSB_TX_MASK));
        }
    }
        ...
        mss_uart_ready_mode_t mode)
    {
        ...
        if (MSS_UART_INVALID_READY_MODE > mode)
        {
            ...
            ((MSS_UART_READY_MODE0 == mode) ? (this_uart->hw_reg->FCR &= ~RDYMODE_MASK) : (this_uart->hw_reg->FCR |= RDYMODE_MASK));
        }
    }
